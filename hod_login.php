...
$mail->Body    = 'The '.$event_name.' is '.$status.' by the HOD';
...
$compare=strtoupper(substr($faculty_email,5,-10));  

$result1=mysqli_query($connect,"select * from `amount` where department='$compare'");
...
$balance=$res1['totalrev']-$res1['utrev'];
 ...
 if(isset($_POST['approve']))
 {
  ...
  $result=mysqli_query($connect,"update `eventlist1` set `hod`='1' where `sdate` ='$i'");
 ...
 if(isset($_POST['reject']))
 {
  $i=$_POST["reject"];
  $i=substr($i,0,10)." ".substr($i,10,18);
  $result=mysqli_query($connect,"update `eventlist1` set `hod`='2' where `sdate` ='$i'");
   $result2=mysqli_query($connect,"select `faculty_email`,`event_title` from `eventlist1` where `sdate` ='$i'");
  $res2=mysqli_fetch_array($result2);
  $to_mail_name=$res2['faculty_email'];
  $event_name=$res2['event_title'];
 ...
 $result=mysqli_query($connect,"select * from `eventlist1` where department='$compare' && hod=0");
